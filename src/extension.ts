...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const d4 = rc('audio-lab.pickModel', async (modelId: string) => {
    if (!modelId) {
  ...
  const d5 = rc('audio-lab.openAudioFile', (fullPath: string) => {
    const uri = vscode.Uri.file(fullPath)
    vscode.commands.executeCommand('vscode.open', uri)
  })
  const d6 = rc('audio-lab.revealInExplorer', (item: vscode.TreeItem) => {
    if (!item.tooltip) return
    const uri = vscode.Uri.file(item.tooltip.toString())
    vscode.commands.executeCommand('revealFileInOS', uri)
  })

  context.subscriptions.push(d1a, d1b, d2, d3, d4, d5, d6)
}
